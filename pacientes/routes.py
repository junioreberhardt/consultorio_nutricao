...
def listar_pacientes():
    ...
    if busca:
        ...
        for acentuado, normal in [
            ...
            nome_normalizado = func.replace(nome_normalizado, acentuado, normal)
        consulta = consulta.filter(nome_normalizado.like(f"%{busca_normalizada}%"))
    consulta = consulta.order_by(Paciente.nome.asc(), Paciente.id.asc())
    total_pacientes = consulta.count()
    total_paginas = max(1, (total_pacientes + por_pagina - 1) // por_pagina)
    ...
    if total_pacientes == 0:
        ...
        fim = min(pagina * por_pagina, total_pacientes)
...
# GERENCIAR PACIENTE
# ==========================================================
@pacientes_bp.route("/gerenciar/paciente/<int:paciente_id>", methods=["GET"])
@login_required
def gerenciar_paciente(paciente_id):
    paciente = Paciente.query.get_or_404(paciente_id)
    anamnese = Anamnese.query.filter_by(paciente_id=paciente.id).first()
    return render_template(
        "pacientes/gerenciar_paciente.html", paciente=paciente, anamnese=anamnese
    )
...
# ==========================================================
@pacientes_bp.route("/editar/paciente/<int:paciente_id>", methods=["GET"])
@login_required
...
# ==========================================================
@pacientes_bp.route("/paciente/<int:paciente_id>/anamnese", methods=["POST"])
@login_required
def salvar_anamnese(paciente_id):
    ...
    anamnese.objetivo_principal = (
        request.form.get("objetivo_principal", "").strip() or None
    )
    anamnese.patologias = request.form.get("patologias", "").strip() or None
    anamnese.medicamentos = request.form.get("medicamentos", "").strip() or None
    anamnese.alergias_intolerancias = (
        request.form.get("alergias_intolerancias", "").strip() or None
    )
    anamnese.aversoes_alimentares = (
        request.form.get("aversoes_alimentares", "").strip() or None
    )
    anamnese.qualidade_sono = request.form.get("qualidade_sono", "").strip() or None
    consumo_agua = request.form.get("consumo_agua_diario", "").strip()
    try:
        if consumo_agua:
...
# ==========================================================
@pacientes_bp.route("/nova/avaliacao/<int:paciente_id>", methods=["GET", "POST"])
@login_required
def nova_avaliacao(paciente_id):
    ...
    if request.method == "POST":
        data_consulta_str = request.form.get("data_consulta", "").strip()

        peso_str = request.form.get("peso_atual", "").strip()

        altura_str = request.form.get("altura_atual", "").strip()

        percentual_gordura_str = request.form.get("percentual_gordura", "").strip()

        percentual_massa_magra_str = request.form.get(
            "percentual_massa_magra", ""
        ).strip()

        cintura_str = request.form.get("circunferencia_cintura", "").strip()

        observacoes = request.form.get("observacoes_clinicas", "").strip()
